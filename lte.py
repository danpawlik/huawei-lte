...
import sys, os, base64, time, datetime, locale, traceback, curses
    ...
    def run(self):
        ...
        while not stop:
            ...
            locale.setlocale(locale.LC_ALL,
                             '')  # sets locale to user default settings
            ...
            dataUsed = int(
                (int(stat["CurrentMonthDownload"]) +
                 int(stat["CurrentMonthUpload"])) / (1024 * 1024 * 1024))
            forfait = int(client.monitoring.start_date()["DataLimit"].replace(
                "GB", "").replace("MB", ""))
            dataAllowed = int(
                (int(stat["MonthDuration"]) / (60 * 60 * 24)) * forfait / 31)
            ...
            win.addstr(
                y, 1,
                date + " - " + basename(sys.argv[0]) + " (" + version + ")",
                curses.color_pair(1))
            y += 2
            ...
            if iPing == -2:
                ...
                win.addstr(y, 28, bar[0:0],
                           curses.color_pair(1))  # Progress bar
            else:
                win.addstr(y, 16, str(iPing + 1), curses.color_pair(1))
                if timePing != 0:
                    win.addstr(y, 20,
                               str(int(timePing * 1000)) + " ms",
                               curses.color_pair(1))
                win.addstr(y, 28, bar[0:(iPing % 10) + 1],
                           curses.color_pair(1))  # Progress bar
            y += 2
            ...
            win.addstr(y, 1, "sinr :         " + str(sinr),
                       curses.color_pair(1))
            y += 1
            win.addstr(y, 1, "cell_id :      " + str(cell_id),
                       curses.color_pair(1))
            y += 1
            win.addstr(y, 1, "plmn :         " + str(plmn),
                       curses.color_pair(1))
            y += 2
            win.addstr(y, 1, "Download :     " + str(download),
                       curses.color_pair(1))
            win.addstr(y, 20, "Mbit/s", curses.color_pair(1))
            win.addstr(y, 28, bar[0:download % 100],
                       curses.color_pair(1))  # Progress bar
            y += 1
            win.addstr(y, 1, "Upload :       " + str(upload),
                       curses.color_pair(1))
            win.addstr(y, 20, "Mbit/s", curses.color_pair(1))
            win.addstr(y, 28, bar[0:upload % 100],
                       curses.color_pair(1))  # Progress bar
            y += 2
            win.addstr(y, 1, "Data allowed : " + str(dataAllowed),
                       curses.color_pair(1))
            win.addstr(y, 20, "Gbyte", curses.color_pair(1))
            win.addstr(y, 28, bar[0:int(dataAllowed / 2)],
                       curses.color_pair(1))  # Progress bar
            y += 1
            win.addstr(y, 1, "Data used :    " + str(dataUsed),
                       curses.color_pair(1))
            win.addstr(y, 20, "Gbyte", curses.color_pair(1))
            win.addstr(y, 28, bar[0:int(dataUsed / 2)],
                       curses.color_pair(1))  # Progress bar
            if delta >= 0:
                y += 1
...
try:
    ...
    if lteband and lteband == "20000000000":
        networkmode = "08"
    else:
        networkmode = "03"
    if band != -1:
